...
func FinishStackOperation(ctx context.Context, op *StackOperation) error {
	return db.WithTx(ctx, func(ctx context.Context) error {
		...
		if op.State == "completed" || op.State == "cancelled" {
			remaining, err := db.GetEngine(ctx).Table(new(StackEntry)).Join("INNER", "pull_request", "pull_request.id = stack_entry.pull_request_id").Where("stack_entry.stack_id = ? AND (pull_request.has_merged = ? OR pull_request.merged_commit_id = ? OR pull_request.merged_commit_id IS NULL)", op.StackID, false, "").Count(new(StackEntry))
			if err != nil {
				return err
			}
			if remaining == 0 {
