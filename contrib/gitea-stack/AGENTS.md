...
7. Check the stack mode in `status --json` (`mode`). In merge mode, `restack` merges parents into layers, `push` never forces and refuses non-fast-forward layers, `rebase --server` starts an update, and `land` accepts only `merge`, `squash` or `fast-forward-only`. On a `non_fast_forward` refusal, run `sync`, merge the remote change, and push again.
