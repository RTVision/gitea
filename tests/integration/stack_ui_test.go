...
func TestNativeStackUpdateBranchButton(t *testing.T) {
	onGiteaRun(t, func(t *testing.T, _ *url.URL) {
		...
		body = session.MakeRequest(t, NewRequest(t, http.MethodGet, fmt.Sprintf("/user2/repo1/pulls/stacks/%d", lower.ID)), http.StatusOK).Body.String()
		assert.Contains(t, body, fmt.Sprintf(`<div class="item" data-value="%d">`, top.Index), "the append link lands on an insert picker offering the pull request")
